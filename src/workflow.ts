...
import { verifyDoi, verifyGds, verifyPatent, verifyPmid, verifySra, verifyTrial, verifyUniprot } from './evidence.ts'
import { checkUrl, type UrlCheckResult } from './urlverify.ts'
import { runSemanticCheck } from './semantic.ts'
import { verifyTitle } from './titleverify.ts'
import { decomposeWithLlm, materializeClaims, mergeClaims, scanReport } from './decompose.ts'
import { createVerificationJob, JobStore, jobStats, refreshFailureAspects, type AspectCheck, type ClaimRecord, type VerificationJob } from './ledger.ts'
...
export function planAspects(job: VerificationJob, claim: ClaimRecord): Array<{ aspect: AspectCheck['aspect']; target?: string }> {
  const plan: Array<{ aspect: AspectCheck['aspect']; target?: string }> = []
  ...
  const hasAnchor = [ids.doi, ids.pmid, ids.nctId, ids.uniprot, ids.geo, ids.sra, ids.patent].some(value => value !== undefined)
    || (ids.urls?.length ?? 0) > 0
  if (!hasAnchor) return [{ aspect: 'identifier_scan' }]
  switch (claim.category) {
    case 'literature':
      if (ids.doi !== undefined) plan.push({ aspect: 'existence', target: `doi:${ids.doi}` })
    ...
    case 'patent':
      if (ids.patent !== undefined) {
        ...
        if (claim.expectedTitle !== undefined) plan.push({ aspect: 'title_agreement', target: `patent:${ids.patent}` })
      }
      break
    case 'clinical_trial':
      if (ids.nctId !== undefined) plan.push({ aspect: 'existence', target: `nct:${ids.nctId}` })
      break
    case 'dataset':
      if (ids.geo !== undefined) {
        plan.push({ aspect: 'existence', target: `geo:${ids.geo}` })
        if (claim.expectedTitle !== undefined) plan.push({ aspect: 'title_agreement', target: `geo:${ids.geo}` })
      }
      if (ids.sra !== undefined) plan.push({ aspect: 'existence', target: `sra:${ids.sra}` })
      if (job.options.semanticChecks && ids.geo !== undefined) plan.push({ aspect: 'semantic_consistency' })
      break
    case 'protein':
      if (ids.uniprot !== undefined) plan.push({ aspect: 'existence', target: `uniprot:${ids.uniprot}` })
      break
    case 'webpage':
      for (const url of ids.urls ?? []) plan.push({ aspect: 'url_accessibility', target: url })
...
function componentName(aspect: AspectCheck['aspect']): string {
  switch (aspect) {
    case 'existence':
    case 'retraction':
...
async function runAspect(deps: VerifyDeps, claim: ClaimRecord, plan: { aspect: AspectCheck['aspect']; target?: string }): Promise<AspectCheck> {
  const startedAt = new Date().toISOString()
  const finish = (status: AspectCheck['status'], detail: string, evidence?: Record<string, unknown>): AspectCheck => ({
    aspect: plan.aspect,
  ...
  const mapCheck = (result: { status: string; detail: string }): AspectCheck['status'] =>
  ...
  if (plan.aspect === 'url_accessibility') {
    ...
    const status: AspectCheck['status'] =
      result.verdict === 'accessible' || result.verdict === 'redirect' ? (result.soft404Suspected ? 'warning' : 'passed')
  ...
  if (plan.aspect === 'semantic_consistency') {
    ...
    const status: AspectCheck['status'] =
      result.verdict === 'consistent' ? 'passed'
  ...
  if (plan.aspect === 'existence') {
    ...
    const result = await (async (): Promise<{ status: string; detail: string } | undefined> => {
      const target = plan.target ?? ''
    ...
    if (result === undefined) return finish('skipped', 'existence planned without a matching identifier')
    return finish(mapCheck(result), result.detail)
  }

  if (plan.aspect === 'retraction') {
    if (ids.pmid === undefined) return finish('skipped', 'retraction scan only applies to PMID citations')
    ...
    return finish(flagged ? 'failed' : 'passed',
      flagged ? '该文献已被撤稿或被标记 Expression of Concern —— 不得作为有效支持引用'
        : '无撤稿/Expression-of-Concern 记录',
      { ...result })
  }

  if (plan.aspect === 'title_agreement') {
    if (claim.expectedTitle === undefined) return finish('skipped', '报告未给出期望标题')
 ...
 */
export async function stepJob(store: JobStore, deps: VerifyDeps, jobId: string): Promise<{
  finished: boolean
  action: string
  summary: string
  remainingChecks: number
  claim?: { claimId: string; status: string; failureAspects: string[] }
}> {
  const job = await store.get(jobId)
  if (job === undefined) return { finished: true, action: 'error', summary: `no verification job "${jobId}"`, remainingChecks: 0 }
...
export async function startVerification(store: JobStore, deps: VerifyDeps, options: {
  report: string
  reportName?: string
...
export function markerFor(claim: ClaimRecord): string | undefined {
  if (claim.status === 'passed' || claim.status === 'pending' || claim.status === 'in_progress') return undefined
...
export function annotateReport(job: VerificationJob): { annotated: string; unanchored: string[] } {
...
export function verificationAppendix(job: VerificationJob): string {
  ...
  lines.push('|---|---|---|---|---|---|---|---|---|')
  for (const claim of job.claims) {
    const cell = (aspect: string): string => {
...
export async function finalizeJob(store: JobStore, deps: { workDir: string }, jobId: string): Promise<{
  ...
  const job = await store.get(jobId)
  if (job === undefined) throw new Error(`no verification job "${jobId}"`)
  job.status = 'annotating'
