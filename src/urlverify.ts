 ...
 */
export async function fetchPageTitle(rawUrl: string, options: { timeoutMs?: number; fetchImpl?: typeof fetch } = {}): Promise<{
  title: string
  finalUrl: string
  httpStatus: number
}> {
...
export async function checkUrl(rawUrl: string, options: UrlCheckOptions = {}): Promise<UrlCheckResult> {
  const url = normalizeUrl(rawUrl)
  const doFetch = options.fetchImpl ?? fetch
  const timeoutMs = options.timeoutMs ?? 12_000
  const started = Date.now()
  const unreachable = (detail: string): UrlCheckResult => ({
...
const VERDICT_ICON: Record<UrlVerdict, string> = {
  accessible: '✅',
  redirect: '↪️',
  blocked: '🔒',
  not_found: '❌',
...
export function buildUrlVerifyTool(): ReturnType<typeof defineTool> {
  return defineTool({
    name: 'url_verify',
    ...
    output: {
      ...
      render: (_args, value: Partial<UrlCheckResult> & { error?: string }) => [{
        type: 'text',
