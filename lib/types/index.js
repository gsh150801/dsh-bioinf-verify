...
import { BioHttpClient } from "./biohttp.js";
import { buildEvidenceTools } from "./evidence.js";
import { buildSemanticCheckTool } from "./semantic.js";
import { buildUrlVerifyTool } from "./urlverify.js";
import { buildTitleVerifyTool } from "./titleverify.js";
import { makeLlmChat } from "./llm.js";
import { JobStore, jobStats } from "./ledger.js";
import { finalizeJob, planAspects, startVerification, stepJob, verificationAppendix } from "./workflow.js";
...
export function apply(ctx, config) {
    ...
    const deps = { client, ncbi, chat, patentsApiKey };
    for (const tool of [
        buildUrlVerifyTool(),
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (report.trim().length < 40)
                return { error: 'report text too short to verify' };
            try {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (job === undefined)
                return { error: `no verification job "${String(args.jobId ?? '')}"` };
