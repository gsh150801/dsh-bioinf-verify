...
import { titleSimilarity } from "./evidence.js";
import { fetchPageTitle } from "./urlverify.js";
const EUTILS_BASE = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils';
export async function fetchTitle(client, ncbi, kind, id, patentsApiKey) {
    if (kind === 'doi') {
        const payload = await client.getJson(`https://api.crossref.org/works/${encodeURIComponent(id)}`, { cacheTtlMs: 600_000 });
        const raw = payload.message?.title;
        const title = Array.isArray(raw) ? String(raw[0] ?? '') : typeof raw === 'string' ? raw : '';
        if (title.trim() === '')
            throw new Error('Crossref record carries no title');
    ...
    if (kind === 'pmid') {
        ...
        for (const [key, value] of Object.entries({ ...common, db: 'pubmed', id, retmode: 'json' }))
            url.searchParams.set(key, value);
        const payload = await client.getJson(url.toString(), { cacheTtlMs: 600_000 });
        const title = String(payload.result?.[id]?.title ?? '');
        if (title.trim() === '')
            throw new Error(`PubMed has no title for PMID ${id}`);
        return { title, provenance: 'PubMed title' };
    }
    if (kind === 'geo') {
        const common = client.ncbiParams(ncbi.apiKey);
        const searchUrl = new URL(`${EUTILS_BASE}/esearch.fcgi`);
        for (const [key, value] of Object.entries({ ...common, db: 'gds', term: `${id}[ACCN]`, retmax: '10', retmode: 'json' }))
            searchUrl.searchParams.set(key, value);
        const searched = await client.getJson(searchUrl.toString());
        const uids = (searched.esearchresult?.idlist ?? []).map(String).filter(uid => /^\d+$/.test(uid));
        if (uids.length === 0)
            throw new Error(`GEO accession ${id} not found`);
        const sumUrl = new URL(`${EUTILS_BASE}/esummary.fcgi`);
        for (const [key, value] of Object.entries({ ...common, db: 'gds', id: uids.join(','), retmode: 'json' }))
        ...
        for (const uid of uids) {
            ...
            if (doc === undefined || String(doc.accession ?? '').toLowerCase() !== wanted)
                continue;
            const title = String(doc.title ?? '');
            if (title.trim() === '')
                throw new Error(`GEO record for ${id} carries no title`);
    ...
    if (kind === 'patent') {
        if (patentsApiKey === '')
            throw new Error('PATENTSVIEW_API_KEY 未配置，无法核验专利标题（免费申请：patentsview.org/apis/keyrequest）');
        const number = id.replace(/^US/i, '').trim();
        const payload = await client.postJson('https://search.patentsview.org/api/v1/patent/', { q: { patent_id: number }, f: ['patent_id', 'patent_title'] }, { headers: { 'x-api-key': patentsApiKey } });
        ...
        if (title.trim() === '')
            throw new Error(`no patent title found for US${number}`);
    ...
    if (page.title.trim() === '')
        throw new Error(`page ${id} exposes no title (HTTP ${page.httpStatus})`);
...
export function titleSimilarityProbe(expected, actual) {
    const sim = titleSimilarity(expected, actual);
    const verdict = sim.contained || sim.jaccard >= 0.75 ? 'match' : sim.jaccard >= 0.4 ? 'close' : 'mismatch';
    return { ...sim, verdict };
}
export async function verifyTitle(client, ncbi, opts, patentsApiKey) {
    ...
    try {
        ...
        const verdict = contained || jaccard >= 0.75 ? 'match' : jaccard >= 0.4 ? 'close' : 'mismatch';
        const label = {
            match: '标题一致',
...
export function buildTitleVerifyTool(client, ncbi, patentsApiKey) {
    return defineTool({
        name: 'title_verify',
...
//# sourceMappingURL=titleverify.js.map