...
export async function fetchSourceText(client, ncbi, kind, id, patentsApiKey) {
    if (kind === 'pmid') {
        const common = client.ncbiParams(ncbi.apiKey);
        const url = new URL(`${EUTILS_BASE}/efetch.fcgi`);
        for (const [key, value] of Object.entries({ ...common, db: 'pubmed', id, rettype: 'abstract', retmode: 'text' }))
    ...
    if (kind === 'doi') {
        ...
        if (abstract.trim() === '')
            throw new Error(`no abstract available via Europe PMC for DOI ${id}`);
    ...
    if (kind === 'geo') {
        ...
        for (const uid of uids) {
            ...
            if (doc === undefined || accession !== wanted)
                continue;
            const summary = String(doc.summary ?? '').trim();
            const title = String(doc.title ?? '').trim();
            if (summary === '' && title === '')
                throw new Error(`GEO record for ${id} carries no description`);
    ...
    if (kind === 'uniprot') {
        const entry = await client.getJson(`https://rest.uniprot.org/uniprotkb/${encodeURIComponent(id)}.json?fields=accession,id,gene_primary,protein_name,organism_name,ft_function`, { cacheTtlMs: 600_000 });
        ...
        const recommended = description?.recommendedName?.fullName;
    ...
    if (payload.error_message !== undefined && payload.error_message !== null)
        throw new Error(`patentsview: ${String(payload.error_message)}`);
    const patent = payload.patents?.[0];
    const abstract = typeof patent?.patent_abstract === 'string' ? patent.patent_abstract : '';
    if (abstract.trim() === '')
        throw new Error(`no abstract found for patent US${number}`);
...
export function semanticUserPrompt(claim, source) {
...
export function parseSemanticReview(text) {
    ...
    if (verdictMatch === null)
        throw new Error(`auditor reply not parseable: ${text.slice(0, 200)}`);
    const verdictKey = verdictMatch[1].toLowerCase().replaceAll(/[_ -]/g, '_');
    const verdict = verdictKey === 'consistent' ? 'consistent'
...
export async function runSemanticCheck(client, ncbi, chat, input, patentsApiKey) {
...
export function buildSemanticCheckTool(client, ncbi, chat, patentsApiKey) {
    return defineTool({
